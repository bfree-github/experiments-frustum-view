...
* Implemented in static HTML5/JavaScript (no node) and straight WebGL (no frameworks).  I'm using jQuery, and ancient WebGL utility lib, and an old matrix lib (time permitting, I may replace this with my own matrix lib).  I'm using 2D canvas for the Overhead view; WebGL of the 3D view.  The two views and Stats are coupled via the parent app.
